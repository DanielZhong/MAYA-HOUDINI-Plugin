...
static PRM_Default angleDefault(30.0);
static PRM_Default stepSizeDefault(3.0);
static PRM_Default iterationsDefault(3.0);
...
SOP_Lsystem::cookMySop(OP_Context& context)
{
	...
	UT_String grammarFileLoad;
	evalString(grammarFileLoad, "grammarFile", 0, now);
	std::string grammar;
	grammar = grammarFileLoad.toStdString();
	...
	if (error() < UT_ERROR_ABORT)
	{
		...
		if (boss->opStart("Building LSYSTEM"))
		{
			...
			{
				const auto& start = branch.first;
				const auto& end = branch.second;

				GA_Offset startPos = gdp->appendPointOffset();
				gdp->setPos3(startPos, UT_Vector3(start[0], start[2], start[1]));

				GA_Offset endPos = gdp->appendPointOffset();
				gdp->setPos3(endPos, UT_Vector3(end[0], end[2], end[1]));

				GU_PrimPoly* poly = GU_PrimPoly::build(gdp, 2, GU_POLY_OPEN, 0);
				poly->setVertexPoint(0, startPos);
				poly->setVertexPoint(1, endPos);
			}
